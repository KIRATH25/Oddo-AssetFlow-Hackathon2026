...
export const CreateBookingModal: React.FC<CreateBookingModalProps> = ({
  ...
  // Load session user and profiles list
  useEffect(() => {
    if (isOpen) {
      // 1. Fetch current profile from cache / session
      ...
      // 2. Fetch list of users for delegation dropdown if manager
      supabase
        .from('profiles')
        .select('id, full_name')
        .eq('status', 'Active')
  ...
  const onSubmit = async (values: BookingFormValues) => {
    // Reconstruct full timestamp intervals
    const startISO = new Date(`${values.date}T${values.start_time}:00`).toISOString()
    const endISO = new Date(`${values.date}T${values.end_time}:00`).toISOString()
            ...
            <DialogPanel className="bg-card dark:bg-zinc-900 border border-border dark:border-zinc-800 rounded-card w-full max-w-[460px] shadow-xl overflow-hidden">
              ...
              >
                ...
                <Description className="sr-only">
                  Form panel to book rooms, vehicles, or equipment.
                </Description>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-20 mt-20">
                  ...
                  {/* Booked on behalf of (Admins/Managers only) */}
                  <div className="flex flex-col gap-6">
